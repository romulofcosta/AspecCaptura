...
namespace pwa_camera_poc_blazor.Services.UnidadesGestoras
{
    ...
    public class UGStateService
    {
        ...
        public UGStateService(HttpClient httpClient, IAuthService authService)
        {
        ...
        /// <summary>
        /// Valida e carrega os dados da UG atual.
        ...
        public async Task ValidateAndLoadAsync()
        {
            ...
            try
            {
                // Tenta carregar o arquivo de inventário da UG
                ...
                {
                    throw new InvalidOperationException("Falha ao deserializar inventário da UG.");
                }
            ...
            {
                Console.Error.WriteLine($"[UGStateService] Erro ao carregar unidade: {ex.Message}");
        ...
        public void EnsureReady()
        {
            if (!IsReady)
            {
                throw new InvalidOperationException("ERR_UG_NOT_LOADED: A Unidade Gestora não foi carregada corretamente.");
            }
        }
    }
}
